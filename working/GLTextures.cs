...
namespace THREE
{
    ...
    public class GLTextures : DisposableObject,IGLTextures
    {
        ...
        private ImageResult ResizeImage(ImageResult image, int width, int height)
        {
            return image;
        ...
        //}
        private ImageResult ResizeImage(ImageResult image, bool needsPowerOfTwo, bool needsNewCanvas, int maxSize)
        {
            float scale = 1.0f;
        ...
        private bool IsPowerOfTwo(ImageResult image)
        {
        ...
        public void SetTexture2D(Texture texture, int slot)
        {
            ...
            if (texture.version > 0 && textureVersion != texture.version)
            {
                ...
                if (texture.Image == null && texture.ImageSize.Width > 0 && texture.ImageSize.Height > 0 && !(texture is DataTexture))
                {
                    ...
                    ImageResult bitmap = new ImageResult();
                    bitmap.Data = data;
                    bitmap.Width = texture.ImageSize.Width;
                    bitmap.Height = texture.ImageSize.Height;
                    bitmap.Comp = ColorComponents.RedGreenBlueAlpha;
        ...
        public void SetTextureCube(Texture texture, int slot)
        {
            ...
            if (texture.version > 0 && textureVersion != texture.version)
            {
                ...
                bool supportsMipsMap = image is ImageResult ? IsPowerOfTwo((ImageResult)image) : IsPowerOfTwo((image as Texture).Image);
                ...
                else
                {
                    ...
                    for (var i = 0; i < 6; i++)
                    {
                        ImageResult localImage = cubeImage[i] as ImageResult;
                        if (isDataTexture)
                        {
                            ...
                            state.TexImage2D((int)targets[i], 0, glInternalFormat, localImage.Width, localImage.Height, 0, (int)glFormat, (int)glType, localImage.Data);
                        ...
                        else
                        {
                            ...
                            GL.TexImage2D((All)targets[i], 0, (All)glInternalFormat, localImage.Width, localImage.Height, 0, localImage.Comp == ColorComponents.RedGreenBlue? All.Rgb:All.Rgba, glType, localImage.Data);
                ...
                if (TextureNeedsGenerateMipmaps(texture, supportsMips))
                {
                    ...
                    GenerateMipmap(TextureTarget.TextureCubeMap, texture, (image as ImageResult).Width, (image as ImageResult).Height);
        ...
        private void UploadTexture(Hashtable textureProperties, Texture texture, int slot)
        {
            ...
            else if (texture is DataTexture)
            {
                ...
                else
                {
                    ...
                    if (texture.Image != null)
                    {
                        ...
                        GL.TexImage2D(All.Texture2D, 0, (All)glInternalFormat, image.Width, image.Height, 0, All.Rgba, glType, image.Data);
            ...
            else
            {
                ...
                else
                {
                    ...
                    //    data.Scan0);
                    GL.TexImage2D(All.Texture2D, 0, (All)glInternalFormat, image.Width, image.Height, 0, glFormat, glType, image.Data);
...
//TODO
// Texture Exampl를 실행하고 CubeTexture Example를 실행하면 Texture 화면이 FlipY=true로 적용된다. Bommot-> top, top->Bottom