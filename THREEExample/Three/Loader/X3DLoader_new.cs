...
namespace THREE
{
    ...
    public class X3DLoader
    {
        ...
        List<float> uvPoints = new List<float>();
        ...
        public void Load(string fileName)
        {
            ...
            Console.WriteLine(scene.ChildNodes[0].Name);
            group = new Object3D();
        ...
        private void parseChildren(XmlNode parentNode, Hashtable parentResult)
        {
            for (var i = 0; i < parentNode.ChildNodes.Count; i++)
            {
                ...
                if (currentNode.NodeType != XmlNodeType.None)
                {
        ...
        private void renderNode(Hashtable data, Object3D parent)
        {
            ...
            if (data.Contains("string"))
            {
                ...
                if (stringValue.Contains(" USE "))
                {
                    ...
                    else
                    {
                        ...
                        {
                            var obj = (Object3D)(defines[defineKey] as Object3D).Clone();
                            obj.Name = (data["parent"] as Hashtable)["name"] as string;
                            parent.Add(obj);
                        }
                    }
                    return;
            ...
            else if ((data["string"]!=null) && (data["string"] as string).Contains("geometry "))
            {
                ...
                else if ("indexedfaceset" == (string)data["nodeType"])
                {
                    ...
                    for (var i = 0; i < (data["children"] as List<Hashtable>).Count; i++)
                    {
                        ...
                        if ("coordinate" == (string)child["nodeType"])
                        {
                            ...
                            if (child.ContainsKey("points"))
                            {
                                ...
                                points = (List<float>)child["points"];
                                geometry.SetAttribute("position", new BufferAttribute<float>(points.ToArray(), 3));
                    ...
                    {
                        List<int> coordIndex = data["coordIndex"] as List<int>;
                        geometry.SetIndex(coordIndex);
                        ...
                        for (var i = 0; i < coordIndex.Count; i++)
                        {
                            ...
                            while (indexes.Count >= 3 && skip < (indexes.Count - 2))
                            {
                                ...
                                if (uvs.Count > 0 && uvIndexes.Count > 0)
                                {
                                    geometry.FaceVertexUvs[0].Add(new List<Vector2> {
                                        new THREE.Vector2(
                                            uvs[uvIndexes[0]].X,
                                            uvs[uvIndexes[0]].Y
                                        ),
                                        new THREE.Vector2(
                                            uvs[uvIndexes[skip +(data["ccw"]!=null && (bool)data["ccw"] ? 1 : 2)]].X,
                                            uvs[uvIndexes[skip + (data["ccw"]!=null && (bool)data["ccw"] ? 1 : 2)]].Y
                                        ),
                                        new THREE.Vector2(
                                            uvs[uvIndexes[skip + (data["ccw"]!=null && (bool)data["ccw"] ? 2 : 1)]].X,
