...
{
    [Example("Controls-Arcball", ExampleCategory.ThreeJs, "Controls")]
    public class ArcballControlsExample : Example
    {
        ...
        private Camera makeOrthographicCamera()
		{
			...
			var halfFovH = (float)Math.Atan((this.Width / this.Height) * Math.Tan(halfFovV));
        ...
        public override void OnResize(ResizeEventArgs clientSize)
        {
			...
			if (camera is OrthographicCamera)
			{
				...
				var halfFovV = MathUtils.DEG2RAD * 45 * 0.5f;
				var halfFovH = (float)Math.Atan((this.Width / this.Height) * (float)Math.Tan(halfFovV));
